...
class JournalPayload(unittest.TestCase):
    ...
    def _close(self, result: str, r: float, inst: str = "BTC-USDT") -> None:
        journal.append({"type": "paper_close", "inst_id": inst, "result": result, "r": r}, strategy="ict")

    def test_trades_survive_a_flood_of_stand_downs(self) -> None:
        journal.append({"type": "paper_fill", "inst_id": "BTC-USDT"}, strategy="ict")
        self._close("win", 2.0)
        for _ in range(80):
            journal.append({"type": "stand_down", "inst_id": "BTC-USDT", "missing": ["amd"]}, strategy="ict")

        snap = journal.snapshot(strategy="ict")
    ...
    def test_trades_are_oldest_first_for_the_equity_curve(self) -> None:
        ...
        results = [e["result"] for e in journal.snapshot(strategy="ict")["trades"]]
        self.assertEqual(results, ["win", "loss"])

    def test_errors_are_counted_and_surfaced(self) -> None:
        journal.append({"type": "error", "inst_id": "ETH-USDT", "error": "boom"}, strategy="ict")
        snap = journal.snapshot(strategy="ict")
    ...
    def test_stats_do_not_carry_a_second_copy_of_open(self) -> None:
        journal.save_open({"BTC-USDT": {"entry": 1, "stop": 0.5, "strategy": "ict"}})
        snap = journal.snapshot(strategy="ict")
    ...
    def test_desk_payload_has_no_duplicated_top_level_book(self) -> None:
        ...
        self.assertEqual(set(payload["books"]), {"desk", "ict", "fabio"})
...
class IncrementalParse(unittest.TestCase):
    ...
    def test_appends_are_picked_up(self) -> None:
        journal.append({"type": "stand_down", "inst_id": "BTC-USDT"}, strategy="ict")
        self.assertEqual(len(journal._parse_lines()), 1)
        journal.append({"type": "stand_down", "inst_id": "ETH-USDT"}, strategy="ict")
    ...
    def test_only_the_tail_is_read_after_an_append(self) -> None:
        for i in range(5):
            journal.append({"type": "stand_down", "inst_id": f"X{i}"}, strategy="ict")
        journal._parse_lines()
        with patch.object(journal, "_decode", wraps=journal._decode) as decode:
            journal.append({"type": "stand_down", "inst_id": "TAIL"}, strategy="ict")
            events = journal._parse_lines()
    ...
    def test_rewritten_file_is_fully_reparsed(self) -> None:
        ...
        journal.invalidate_cache()
        self.assertEqual(journal._parse_lines(), [])
